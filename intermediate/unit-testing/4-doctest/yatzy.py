r"""
We can use doctest to check if the examples given on a method's docstring 
apply to what the code actually does. 

    '$ python3 -m doctest file_name \.py -v'
    '$ python3 -m pytest --doctest-modules'

Give global directives:
    '$ python3 -m doctest -v -o IGNORE_EXCEPTION_DETAIL file_name.py'
    '$ python3 -m pytest --doctest-modules -o doctest_optionflags=IGNORE_EXCEPTION_DETAIL'
"""
...
# Failure scenario
def small_straight_fail_doctest(dice):
    """Score the given roll in the 'small straight' yatzy category.
    >>> small_straight_fail_doctest([1, 2, 3, 4, 5])
    15
    >>> small_straight_fail_doctest([1, 1, 3, 3, 5])
    0

    It doesn't handle sets but it does now handle sorted lists (example was not updated)

    >>> small_straight_fail_doctest({1, 2, 3, 4, 5})
    0
    >>> small_straight_fail_doctest([1, 2, 3, 5, 4])
    0
    """
    if sorted(dice) == [1, 2, 3, 4, 5]:
        return sum(dice)
